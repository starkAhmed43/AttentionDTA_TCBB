...
def train_model(config=None):
    ...
    for i, (train_indices, test_indices) in enumerate(k_fold_splits):
        ...
        wandb_logger = WandbLogger(
            project=f"attn_dta_pred_{config.dataset}_{config.evaluation_type}",
        ...
        trainer = pl.Trainer(
            ...
            callbacks=[pl.callbacks.ModelCheckpoint(monitor="val_mse")],  # Monitor validation MSE for checkpointing
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Dataset to use for training (options: davis, kiba, metz)",
    )
    parser.add_argument(
        "--dataset_path", "-dp",
        type=str,
    ...
    sweep_id = wandb.sweep(
        sweep_config,
        project=f"attn_dta_pred_{args.dataset}_{args.evaluation_type}",  # Project name based on the dataset
    )
